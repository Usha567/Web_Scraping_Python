...
try:
    tab = driver.find_element(By.CSS_SELECTOR, "ul#tractorbrands  li>a#Harvesters-tab")
    print('tab-', tab)
    driver.execute_script("arguments[0].scrollIntoView(true);", tab)
except ElementClickInterceptedException as e:
    print('intercepted...',e)
    # driver.execute_script("arguments[0].click();", brand_btn)   
# driver.execute_script("window.scrollTo(0, "+tab+")")
...
#     try:
        ...
        # image_list
brand_divs = driver.find_elements(By.CSS_SELECTOR, 'div#brandstabcontent div#Harvesters div.tjcol.states-block')
...
# brand = driver.find_element(By.CSS_SELECTOR, "div.brand-main>a>p").text
# brand_list.append(brand)  
print('brand_list-///',brand_list)      
tractor_images = driver.find_elements(By.CSS_SELECTOR, "div#brandstabcontent  div#Harvesters div.tjcol.states-block div.brand-main>a>img")
src =[]

print('harvester_brnd_images-', len(tractor_images))
...
# print('src//- ', src)
    ...
    # if '/' in model:
    #     m= model.split('/')
    #     dirname = "Brand_Images/"+((brand.capitalize()).strip())+"_"+str(i)+"_"+m[0]+"_"+m[1]
    #     print('dirname-', dirname)
    # else:
    # dirname = "Brand_Images/"+((brand.capitalize()).strip())+"_"+str(i)
    # print('dirname-', dirname)     
    # try:
    #     os.mkdir(dirname)
    # except OSError as e:
...
for i in range((len(src))):
    ...
    if(src[i] != ''):
        ...
        # urllib.request.urlretrieve(str(src[i]), "Harvester_brand_image/"+img_name+'.png'.format(i))
...
writer = pd.ExcelWriter('harvester_brand_Images.xlsx', engine='xlsxwriter')
