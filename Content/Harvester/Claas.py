...
driver.get('https://www.tractorjunction.com/tractor-combine-harvesters/?shortBy=&harbrand=94&cutting=&power=')
...
for i in range(2,8):
    ...
    try:
        ...
        for tr in tr_list:
            ...
            if  (text).strip() == 'CUTTER BAR' or (text).strip() == 'CUTTER BAR MECHANISM' or (text).strip() == 'Cutter-bar': 
                index = tr_list.index(tr)
                print('index-',index,  tr.find_element(By.CSS_SELECTOR,  "td:nth-child(1)").text)
            ...
            if  (text).strip() == 'THRESHING EQUIPMENT' or  (text).strip() == 'THRESHING DRUM' or (text).strip() == 'Threshing System': 
                index = tr_list.index(tr)
                print('index-',index,  tr.find_element(By.CSS_SELECTOR,  "td:nth-child(1)").text)
            ...
            if  (text.capitalize()).strip() == 'Tyres' or (text.capitalize()).strip() =='Tyre Sizes': 
                index = tr_list.index(tr)
                print('index-',index,  tr.find_element(By.CSS_SELECTOR,  "td:nth-child(1)").text)
                for i in range(index+1, index+3):
                    try:
        ...
        cylinder=[]
        if 'No.of Cylinder' in engine_text_list or 'Cylinders/Displacements' in engine_text_list or 'Cylinders' in engine_text_list:
        ...
        if 'Ground Clearance' in tyre_text_list or 'Ground Clearance' in dimension_text_list:
...
# writer = pd.ExcelWriter('Claas.xlsx', engine='xlsxwriter')
