...
driver.get('https://www.tractorjunction.com/tractor-combine-harvesters/?shortBy=&harbrand=214&cutting=&power=')
...
for i in range(1,3):
    ...
    try:
        ...
        for tr in tr_list:
            ...
            text = (tr_text.text).lstrip()
            print('text is - ', text)
            if (text.capitalize()).strip() == 'Engine':
            ...
            if  (text).strip() == 'Overall Dimension' or (text).strip() == 'Dimensions' or (text).strip() == 'Overall Dimension (mm)': 
                index = tr_list.index(tr)
                print('index-',index,  tr.find_element(By.CSS_SELECTOR,  "td:nth-child(1)").text)
                for i in range(index+1, index+5):
                    try:
        ...
        if 'Width (mm)' in threshing_text_list or 'Width' in threshing_text_list:
        ...
        if 'Outside dia (mm)' in threshing_text_list or 'Diameter' in threshing_text_list:
...
writer = pd.ExcelWriter('Bakhsish_Harvester.xlsx', engine='xlsxwriter')
