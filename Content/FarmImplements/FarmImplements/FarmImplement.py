...
if buttonText == 'Load More Implements':
    ...
    for i in range(1,2):
        ...
        try:
            ...
            overall_lengthtext_list=[]
            overall_lengthans_list=[]

            overall_widthtext_list=[]
            overall_widthans_list=[]

            overall_height_list=[]
            overall_heightans_list=[]
            ...
            for tr in tr_list:
                try:
                    ...
                    if  (text).strip() == 'Overall Dimension' or (text).strip() =='Overall dimensions' or (text).strip() == 'Dimensions' or (text).strip() == 'Dimension' or (text).strip() == 'DIMENSIONS' or (text).strip() == 'Overall Dimension (mm)': 
                        index = tr_list.index(tr)
                        print('index-',index,  tr.find_element(By.CSS_SELECTOR,  "td:nth-child(1)").text)
                        for i in range(index+1, index+4):
                            try:
                    ...
                    if text.strip() == 'Overall Length':
                        ...
                        for i in range(index+1, index+2):
                            try:
                                ...
                                if new_tr_text !='':
                                    if ':' in new_tr_text: 
                                        overall_lengthtext_list.append((new_tr_text[:-1]).strip())
                                    else:
                                        overall_lengthtext_list.append((new_tr_text).strip())    
                                else:
                                    overall_lengthtext_list.append('')

                                if new_tr_ans_text !='':   
                                    overall_lengthans_list.append(new_tr_ans_text)
                                else:
                                    overall_lengthans_list.append('')
                            except IndexError as e:
                                print('Index error...')        
                            except NoSuchElementException as e:
                                print('No such element')     

                    if text.strip() == 'Overall Width':
                        index = tr_list.index(tr)
                        for i in range(index+1, index+2):
                            try:
                                new_tr_text = (tr_list[i].find_element(By.CSS_SELECTOR, "td:nth-child(1)").text).strip()
                                ...
                                if new_tr_text !='':
                                    if ':' in new_tr_text: 
                                        overall_widthtext_list.append((new_tr_text[:-1]).strip())
                                    else:
                                        overall_widthtext_list.append((new_tr_text).strip())    
                                else:
                                    overall_widthtext_list.append('')

                                if new_tr_ans_text !='':   
                                    overall_widthans_list.append(new_tr_ans_text)
                                else:
                                    overall_widthans_list.append('')
                            except IndexError as e:
                                print('Index error...')        
                            except NoSuchElementException as e:
                                print('No such element')  
                
                    if text.strip() == 'Overall Height':
                        index = tr_list.index(tr)
                        for i in range(index+1, index+2):
                            try:
                                new_tr_text = (tr_list[i].find_element(By.CSS_SELECTOR, "td:nth-child(1)").text).strip()
                                ...
                                if new_tr_text !='':
                                    if ':' in new_tr_text: 
                                        overall_height_list.append((new_tr_text[:-1]).strip())
                                    else:
                                        overall_height_list.append((new_tr_text).strip())    
                                else:
                                    overall_height_list.append('')

                                if new_tr_ans_text !='':   
                                    overall_heightans_list.append(new_tr_ans_text)
                                else:
                                    overall_heightans_list.append('')
                            except IndexError as e:
                                print('Index error...')        
                            except NoSuchElementException as e:
                                print('No such element')  

                    if text.strip() == 'Hitch Type':
                        index = tr_list.index(tr)
                        for i in range(index+1, index+2):
                            try:
                                new_tr_text = (tr_list[i].find_element(By.CSS_SELECTOR, "td:nth-child(1)").text).strip()
                                ...
                                if new_tr_text !='':
                                    if ':' in new_tr_text: 
                                        hitch_list.append((new_tr_text[:-1]).strip())
                                    else:
                                        hitch_list.append((new_tr_text).strip())    
                                else:
                                    hitch_list.append('')

                                if new_tr_ans_text !='':   
                                    hitch_ans_list.append(new_tr_ans_text)
                                else:
                                    hitch_ans_list.append('')
                            except IndexError as e:
                                print('Index error...')        
                            except NoSuchElementException as e:
                                print('No such element')  
                    
                    if text.strip() == 'No. of Tine':
                        index = tr_list.index(tr)
                        for i in range(index+1, index+2):
                            try:
                                new_tr_text = (tr_list[i].find_element(By.CSS_SELECTOR, "td:nth-child(1)").text).strip()
                                ...
                                if new_tr_text !='':
                                    if ':' in new_tr_text: 
                                        tine_list.append((new_tr_text[:-1]).strip())
                                    else:
                                        tine_list.append((new_tr_text).strip())    
                                else:
                                    tine_list.append('')

                                if new_tr_ans_text !='':   
                                    tine_ans_list.append(new_tr_ans_text)
                                else:
                                    tine_ans_list.append('')
                            except IndexError as e:
                                print('Index error...')        
                            except NoSuchElementException as e:
                                print('No such element')  
                    
                    if text.strip() == 'Type of Tine':
                        index = tr_list.index(tr)
                        for i in range(index+1, index+2):
                            try:
                                new_tr_text = (tr_list[i].find_element(By.CSS_SELECTOR, "td:nth-child(1)").text).strip()
                                ...
                                if new_tr_text !='':
                                    if ':' in new_tr_text: 
                                        tinetype_list.append((new_tr_text[:-1]).strip())
                                    else:
                                        tinetype_list.append((new_tr_text).strip())    
                                else:
                                    tinetype_list.append('')

                                if new_tr_ans_text !='':   
                                    tinetype_ans_list.append(new_tr_ans_text)
                                else:
                                    tinetype_ans_list.append('')
                            except IndexError as e:
                                print('Index error...')        
                            except NoSuchElementException as e:
                                print('No such element')  
            ...
            if 'Overall Length' in overall_lengthtext_list:
                index_of = overall_lengthtext_list.index('Overall Length')
                text=overall_lengthans_list[index_of]
                dimensions_length.append(text)  
            else:
                dimensions_length.append('')

            if 'Overall Width' in overall_widthtext_list:
                index_of = overall_widthtext_list.index('Overall Width')
                text=overall_widthans_list[index_of]
                dimensions_width.append(text)  
            else:
                dimensions_width.append('')

            if 'Overall Height' in overall_height_list:
                index_of = overall_height_list.index('Overall Height')
                text=overall_heightans_list[index_of]
                dimensions_height.append(text)  
            else:
                dimensions_height.append('')

            if 'Hitch Type' in hitch_list:
                index_of = hitch_list.index('Hitch Type')
                text=hitch_ans_list[index_of]
                hitch_type.append(text)  
            else:
                hitch_type.append('')    

            if 'No. of Tine' in tine_list:
                index_of = tine_list.index('No. of Tine')
                text=tine_ans_list[index_of]
                no_of_tine.append(text)  
            else:
                no_of_tine.append('') 

            if 'Type of Tine' in tinetype_list:
                index_of = tinetype_list.index('No. of Tine')
                text=tinetype_ans_list[index_of]
                type_of_tine.append(text)  
            else:
                type_of_tine.append('')
